...
final class QuestionClassifierTests: XCTestCase {
    ...
    func test阅读理解标题词句接语细节意图主旨() {
        ...
        XCTAssertEqual(rec("言语理解", "阅读理解", "这段文字意在强调"), "主旨与意图")
        XCTAssertEqual(rec("言语理解", "阅读理解", "这段文字主要说明了"), "主旨与意图")
        XCTAssertEqual(rec("言语理解", "阅读理解", "这段文字谈论的核心问题是"), "主旨与意图")
    }

    func test阅读理解主旨与意图优先于主旨与意图() {
        XCTAssertEqual(rec("言语理解", "阅读理解", "这段文字主要说明的是……意在强调……"), "主旨与意图")
    }

    func test语句表达病句排序接语词语填空衔接() {
        XCTAssertEqual(rec("言语理解", "语句表达", "下列各句中，没有语病的一句是"), "病句辨析")
        // 错别字规则已删(全库 1 题,不足独立成行) — 无规则可归纳 → 其他。
        XCTAssertEqual(rec("言语理解", "语句表达", "下列句子中没有错别字的是"), "词语填空")
        ...
        XCTAssertEqual(rec("言语理解", "语句表达", "以下哪句话最适合放在文中横线处？"), "接语选择")
    }
    ...
    func test数字推理幂次分数多重数阵拆分递推多级() {
        ...
        XCTAssertEqual(rec("数字运算", "数字推理", "下图数阵中问号处应填（ ）", "数字拆分"), "数字拆分")
    ...
    func test数字运算巧算整除数位方程() {
        // 定义新运算规则已删(全库 2 题,均为解方程类) — 依题面归入巧算/兜底。
        XCTAssertEqual(rec("数字运算", "数字运算", "定义一种新的运算a※b=2a+b，则3※4的值为"), "巧算与速算")
        XCTAssertEqual(rec("数字运算", "数字运算", "规定一种新运算a△b=3a-2b，则5△2=？"), "方程与和差倍比")
        XCTAssertEqual(rec("数字运算", "数字运算", "计算：9999×9999的值为"), "巧算与速算")
        XCTAssertEqual(rec("数字运算", "数字运算", "一个数除以7余3，除以5余2，这个数最小是"), "整除与余数")
        XCTAssertEqual(rec("数字运算", "数字运算", "一个两位数，个位数字是十位数字的2倍，这个数可能是"), "数字拆分")
    ...
    func test图形推理空间属性位置样式数量其他() {
        ...
        XCTAssertEqual(rec("逻辑推理", "图形推理", "与众不同的图形是", ""), "样式规律")
        XCTAssertEqual(rec("逻辑推理", "图形推理", "观察图形，选择符合要求的", ""), "属性规律")
    }

    func test类比推理语义逻辑语义关系() {
        XCTAssertEqual(rec("逻辑推理", "类比推理", "雪花对于（）相当于（）对于光泽", "近义关系"), "语义关系")
        XCTAssertEqual(rec("逻辑推理", "类比推理", "鸽子对于（）相当于（）对于蓝色", "种属关系"), "逻辑关系")
        XCTAssertEqual(rec("逻辑推理", "类比推理", "（）对于认真相当于对于负责", "词性均为形容词"), "语义关系")
    }

    func test定义判断统一归定义判断() {
        // 多定义规则已删(全库 1 题);section 只剩兜底,统一为 section 名。
        XCTAssertEqual(rec("逻辑推理", "定义判断", "根据上述定义", "多定义题"), "定义判断")
        XCTAssertEqual(rec("逻辑推理", "定义判断", "根据上述定义，下列不属于训练的是"), "定义判断") // fallback
    }
