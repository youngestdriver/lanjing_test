...
object QuestionClassifier {
    ...
    private val sectionRules: Map<String, List<Rule>> = mapOf(
        ...
        "言语理解|阅读理解" to listOf(
            ...
            rule("主旨与意图", "意在|想要|想表达|旨在|启示|告诉|意图|强调|反映.*问题|主要说明|主要讲述|主要谈论|主旨|概括|中心|主要观点|主要内容|说明的是|谈论|说明了|主要介绍|介绍了|复述|关键词|重在"),
        ),
        "言语理解|语句表达" to listOf(
        ...
        "数字运算|数量关系" to listOf(
            ...
            rule("日期与周期", "钟表|时针|分针|钟面|敲钟|挂钟"),
            // 收紧:裸"植树/间隔/棵/棵数"会误伤非间隔题(树苗配比、客房隔间)。
            // 只留明确的植树/间隔题面,其余落入和差倍比与方程。鸡兔同笼规则
            // 已删(全库 1 题,由方程类兜底)。
            rule("植树与间隔", "植树问题|两端(都|均).{0,4}(种|栽)|间隔数|电线杆|每隔.{0,3}(种|栽)|路边.{0,3}(种|栽)|路旁.{0,3}(种|栽)"),
            rule("牛吃草问题", "牛吃草|牧草|长草|吃草"),
            rule("和差倍比与方程", "平均分|平均数|平均成绩|总平均"),
        ...
        "逻辑推理|图形推理" to listOf(
            ...
            rule("样式规律", "样式规律|求同|求异|叠加|黑白运算|去同存异|去异存同|图形间关系|与众不同"),
        ...
        "逻辑推理|类比推理" to listOf(
            ...
            rule("语义关系", "词性|动宾|主谓|偏正|语法|成语结构|名词|形容词|修饰|主宾"),
        ),
        // 定义判断只有 多定义/单定义 两种;多定义规则已删(全库 1 题),
        // 全部落入 定义判断 兜底 — key 保留为空表(缺失 key 等价,但保持三端一致)。
        "逻辑推理|定义判断" to emptyList(),
    )
