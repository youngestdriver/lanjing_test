...
class QuestionClassifierTest {
    ...
    fun `阅读理解 标题词句接语细节意图主旨`() {
        ...
        assertEquals("主旨与意图", rec("言语理解", "阅读理解", "这段文字意在强调"))
        assertEquals("主旨与意图", rec("言语理解", "阅读理解", "这段文字主要说明了"))
        assertEquals("主旨与意图", rec("言语理解", "阅读理解", "这段文字谈论的核心问题是"))
    }

    @Test
    fun `阅读理解 主旨与意图优先于主旨与意图`() {
        assertEquals("主旨与意图", rec("言语理解", "阅读理解", "这段文字主要说明的是……意在强调……"))
    }

    @Test
    fun `语句表达 病句排序接语词语填空衔接`() {
        assertEquals("病句辨析", rec("言语理解", "语句表达", "下列各句中，没有语病的一句是"))
        // 错别字规则已删(全库 1 题,不足独立成行) — 无规则可归纳 → 其他。
        assertEquals("词语填空", rec("言语理解", "语句表达", "下列句子中没有错别字的是"))
        ...
        assertEquals("接语选择", rec("言语理解", "语句表达", "以下哪句话最适合放在文中横线处？"))
    }
    ...
    fun `数字推理 幂次分数多重数阵拆分递推多级`() {
        ...
        assertEquals("数字拆分", rec("数字运算", "数字推理", "下图数阵中问号处应填（ ）", "数字拆分"))
    ...
    fun `数字运算 巧算整除数位方程`() {
        ...
        assertEquals("巧算与速算", rec("数字运算", "数字运算", "定义一种新的运算a※b=2a+b，则3※4的值为"))
        assertEquals("方程与和差倍比", rec("数字运算", "数字运算", "规定一种新运算a△b=3a-2b，则5△2=？"))
        assertEquals("巧算与速算", rec("数字运算", "数字运算", "计算：9999×9999的值为"))
        assertEquals("整除与余数", rec("数字运算", "数字运算", "一个数除以7余3，除以5余2，这个数最小是"))
        assertEquals("数字拆分", rec("数字运算", "数字运算", "一个两位数，个位数字是十位数字的2倍，这个数可能是"))
    ...
    fun `图形推理 空间属性位置样式数量其他`() {
        ...
        assertEquals("样式规律", rec("逻辑推理", "图形推理", "与众不同的图形是", ""))
        assertEquals("属性规律", rec("逻辑推理", "图形推理", "观察图形，选择符合要求的", ""))
    }
    ...
    fun `类比推理 语义逻辑语义关系`() {
        ...
        assertEquals("语义关系", rec("逻辑推理", "类比推理", "（）对于认真相当于对于负责", "词性均为形容词"))
    }

    @Test
    fun `定义判断 统一归定义判断`() {
        // 多定义规则已删(全库 1 题);section 只剩兜底,统一为 section 名。
        assertEquals("定义判断", rec("逻辑推理", "定义判断", "根据上述定义", "多定义题"))
        assertEquals("定义判断", rec("逻辑推理", "定义判断", "根据上述定义，下列不属于训练的是")) // fallback
    }
