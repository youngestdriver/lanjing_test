...
// Verified against the full bank: 其他 4 题 (0.1%) — 各类型题量 ≥3,练习页
// 不会再出现只有一两题的题型(零散小类型已并入相邻大类型/其他)。
//
...
const SECTION_RULES = {
  ...
  "言语理解|阅读理解": [
    ...
    { name: "主旨与意图", patterns: [/意在|想要|想表达|旨在|启示|告诉|意图|强调|反映.*问题|主要说明|主要讲述|主要谈论|主旨|概括|中心|主要观点|主要内容|说明的是|谈论|说明了|主要介绍|介绍了|复述|关键词|重在/] },
  ],
  "言语理解|语句表达": [
    ...
    { name: "接语选择", patterns: [/横线|填入.*句子|衔接|承上启下|空格|语意连贯|括号/] },
  ],
  "数字运算|数字推理": [
    ...
    { name: "数字拆分", patterns: [/图形数阵|数阵|图中|图形|机械划分|因式分解|小数数列|拆分|小数点|个位.*十位|数位/] },
  ...
  "数字运算|数量关系": [
    ...
    { name: "日期与周期", patterns: [/钟表|时针|分针|钟面|敲钟|挂钟/] },
    // 收紧:裸"植树/间隔/棵/棵数"会误伤非间隔题(树苗配比、客房间隔)。
    // 只留明确的植树/间隔题面,其余落入和差倍比与方程(全库此类仅 0~2 题,
    // 不值得独立成行)。
    { name: "植树与间隔", patterns: [/植树问题|两端(都|均).{0,4}(种|栽)|间隔数|电线杆|每隔.{0,3}(种|栽)|路边.{0,3}(种|栽)|路旁.{0,3}(种|栽)/] },
    { name: "牛吃草问题", patterns: [/牛吃草|牧草|长草|吃草/] },
    { name: "和差倍比与方程", patterns: [/平均分|平均数|平均成绩|总平均/] },
  ...
  "数字运算|数字运算": [
    ...
    { name: "数字拆分", patterns: [/三位数|两位数|四位数|个位|十位|百位|位数|数字|页码|书页|号码|数位/] },
  ],
  "逻辑推理|逻辑判断": [
  ...
  "逻辑推理|图形推理": [
    ...
    { name: "样式规律", patterns: [/样式规律|求同|求异|叠加|黑白运算|去同存异|去异存同|图形间关系|与众不同/] },
  ...
  "逻辑推理|类比推理": [
    ...
    { name: "语义关系", patterns: [/词性|动宾|主谓|偏正|语法|成语结构|名词|形容词|修饰|主宾/] },
  ],
  // 定义判断 only has 多定义/单定义 two flavors; with the 多定义 rule gone
  // (全库 1 题), everything falls to the 定义判断 fallback — the key must
  // stay present (empty rules → fallback; a missing key would return null and
  // misroute to 其他).
  "逻辑推理|定义判断": [],
};
