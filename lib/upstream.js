...
// arrangement where the collector CLI booted the local web proxy and reused
// its session: this module talks to the upstream platform (lanjingweike.com)
// directly with its own cookie jar,
...
// lib/question-bank.js), and mirrors the proxy behavior of the former web
// server (the separate lanjing-web repo):
...
//
// The session is persisted to <sessionFile> (default data/session_cookies.txt,
// mode 0o600) after login and reused on later runs. No
...
function createUpstreamApi(opts = {}) {
  ...
  // ========== low-level HTTP (ported from the former web server) ==========
  ...
  // ========== exam flows (ported from the former web server) ==========
