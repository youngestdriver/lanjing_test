...
| 自动化验证 | 51 项 Node 单元测试；真实 Chrome + mock API 的浏览器回归；无真实上游 E2E | 9 个 XCTest 套件、65 项单元测试；尚无 UI、真机或真实上游 E2E |
