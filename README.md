...
| 自动化验证 | 19 项 Node 单元测试；真实 Chrome + mock API 的浏览器回归；无真实上游 E2E | 8 个 XCTest 套件、56 项单元测试；尚无 UI、真机或真实上游 E2E |

Web 额外提供桌面浏览器入口、响应式布局、触控和键盘答题、主题切换以及可安装的 PWA 应用壳。iOS 提供原生分页手势、iPad 键盘导航、原生富文本容器和系统级会话存储。iOS 的“练习”页目前只是返回考试列表的入口，不是独立练习题库。
...
> [!WARNING]
> Web 后端使用进程级全局 Cookie 和缓存，只适合本机单用户运行。服务默认仅监听 `127.0.0.1`，并拒绝非本地主机、跨源和非 JSON 的写请求，但仍没有多用户会话隔离、TLS、CSRF token 或限流。不要通过反向代理把它暴露到局域网或公网，也不要共享 `apps/web/.local/` 和包含会话信息的终端日志。
...
当前 19 项单元测试覆盖历史答案映射、考试页与成绩页解析、会话失效识别、多选集合判定、答案编码、下一未答题、陈旧考试抑制，以及本地 API 的 Host、Origin、JSON 写请求、退出登录和旧上游响应隔离。浏览器回归会启动本地服务和真实 Chrome，并完全拦截 `/api/*`：它验证多选提交、历史答案恢复、同步失败重试、交卷与旧 `401` 响应的竞态隔离、成绩页终态、键盘和触控交互、自动切题设置持久化，以及桌面、390px、320px 和低高度横屏布局。两类测试都不访问真实上游；浏览器回归需要系统已安装 Google Chrome，或通过 `CHROME_PATH` 指定兼容的 Chromium 可执行文件。
...
- `Node`：Ubuntu、Node 22、依赖安装、JavaScript 语法检查、19 项单元测试、mock API 浏览器回归和 `/api/status` smoke test
