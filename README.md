...
面向蓝鲸微课考试流程的第三方学习客户端。本仓库包含共享业务目标和上游协议的两套独立实现。Web/PWA 是较早的浏览器参考实现；原生 iOS 在功能完整度、代码结构和自动化验证方面更成熟。两端不共享运行时、会话或业务代码，也不承诺逐项功能与行为完全一致。
...
|---|---|---|---|---|
| 原生 iOS | 功能与工程保障更完整的 SwiftUI 客户端 | `ios/` | Xcode 构建并安装到模拟器或设备 | iOS Keychain |
| Web / PWA | 较早的浏览器参考实现和本地使用入口 | `frontend/`、`server.js` | 浏览器访问本地 Express 服务 | Node 进程内存与根目录 `session_cookies.txt` |
...
Web 版必须启动仓库根目录的 Node.js 服务。iOS 版直接访问上游，不依赖 `server.js`，也不需要同时运行 Web 服务。Web 可用于浏览器访问和对照上游流程，但不能视为 iOS 的等价后备客户端。

## 功能与差距

两端都覆盖登录、考试列表、开始或继续考试、逐题作答、题目标记、答题卡、交卷和结果展示等核心场景。这一共享业务范围不代表两端具有相同的交互、可靠性或测试覆盖。

| 能力 | Web / PWA | 原生 iOS |
|---|---|---|
| 客户端形态 | 响应式单页应用、可安装 PWA，无前端构建步骤 | SwiftUI 原生应用，支持 iPhone 与 iPad |
| 单选题 | 点击选项后立即判定并提交 | 点击选项后立即判定并提交 |
| 多选题 | 仅显示题型；点击第一个选项即锁定并只上报一个答案 | 可选择多个选项、确认后按完整集合判定和上报 |
| 历史作答 | 恢复正确、错误和未答状态，不展示用户之前的具体选择 | 恢复状态及用户之前选择的选项 |
| 自动切题 | 答对后固定延迟切题，不能关闭 | 用户可配置，默认关闭，并防止手动切题后的过期跳转 |
| 列表与失败恢复 | 基础加载和错误文本 | 下拉刷新、空态、加载失败重试及放弃考试后的陈旧记录抑制 |
| 答案上报失败 | 请求不阻塞界面，网络异常被静默忽略，本地与上游状态可能不一致 | 捕获请求错误并统一处理会话失效；其他上报失败仍不会回滚本地判题状态 |
| 会话持久化 | 进程级全局 Cookie，并明文写入 `session_cookies.txt` | Cookie 存入 Keychain，会话失效时统一清理并返回登录页 |
| 自动化验证 | 没有业务单元测试或浏览器 E2E；CI 仅做语法和启动检查 | 8 个 XCTest 套件、56 项单元测试；尚无 UI、真机或真实上游 E2E |

Web 仍提供桌面浏览器入口、响应式布局、触控滑动、键盘答题、主题切换和可安装的 PWA 应用壳。iOS 额外提供原生分页手势、iPad 键盘导航、富文本与相对图片渲染、主题和自动切题设置。iOS 的“练习”页目前只是返回考试列表的入口，不是独立练习题库。
